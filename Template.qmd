...
subtitle: "11.25.24 - 12.1.24 vs. 11.24.25 - 11.30.25"
theme: _brand.yml
format: html
---
...
create_pass_comparison <- function(data) {
  ...
  # Helper function for percent change (returns character)
  calc_pct_change <- function(old, new) {
    case_when(
      old == 0 & new == 0 ~ "0",
      old == 0 ~ "New Pass Visits",
      TRUE ~ as.character(round((new / old - 1) * 100, 1))
    )
  }
  ...
  totals <- data |>
    summarize(
      Pass_Type = "Total",
      Visits24 = sum(Visit24, na.rm = TRUE),
      Visits25 = sum(Visit25, na.rm = TRUE),
      PercentChange = calc_pct_change(Visits24, Visits25)
    )
  ...
  by_pass <- data |>
    ...
    mutate(PercentChange = calc_pct_change(Visits24, Visits25))
...
create_plot <- function(data, title, y_limit, subtitle = "Last Week of November") {
...
TotalPlotFinal <- create_plot(FullData, "Total Pass Usage by Day of the Week", 4000)
SouthPlot <- create_plot(SouthRec, "South Rec Pass Usage by Day of the Week", 3000)
NorthPlot <- create_plot(NorthRec, "North Rec Visits by Day of the Week", 1250)
BDPlot <- create_plot(BDRec, "Bear Down Rec Pass Usage by Day of the Week", 125)
S91Plot <- create_plot(S91, "Studio 91 Pass Usage by Day of the Week", 35)
UnassignedPlot <- create_plot(Unassigned, "Unassigned Pass Usage by Day of the Week", 20) 

SenSource <- tibble(Facility = "SenSource", Visits24 = "Coming Soon", Visits25 = "Coming Soon", PercentChange = "Coming Soon")
...
facility_overview <- bind_rows(
  ...
  BDRECPassCompare |> filter(Pass_Type == "Total") |> mutate(Facility = "Bear Down Rec"),
  S91PassCompare |> filter(Pass_Type == "Total") |> mutate(Facility = "Studio 91"),
  
) |>
  select(Facility, Visits24, Visits25, PercentChange) |> 
  mutate(across(c(Visits24, Visits25, PercentChange), as.character)) |> 
  bind_rows(SenSource)
...
datatable(
  ...
  formatStyle(
    ...
    color = styleInterval(
      cuts = c(-25, 25),
      values = c("#EF4056", "black", "#70B865")
    )
  ) |>
  formatStyle(        
    columns = 1:ncol(facility_overview),
    target = "row",
    fontWeight = styleEqual("Total Campus", "bold"),
...
```{r echo=FALSE}
#| label: manual SenSource data

SenSource <- bind_rows(
  tibble(Facility = "SenSource", Visit24 = "10000", Visit25 = "12000", PercentChange = "10.10"))
